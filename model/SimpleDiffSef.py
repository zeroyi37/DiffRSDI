...
class CondGaussianDiffusion(GaussianDiffusion):
    ...
    def forward(self, img, cond_img, depth, seg=None, extra_cond=None, *args, **kwargs):
        ...
        return self.p_losses(img, times, cond_img, depth, seg, extra_cond, *args, **kwargs)

    def p_losses(self, x_start, times, cond_img, depth, seg=None, extra_cond=None, noise=None, *args, **kwargs):
        ...
        model_out = self.model(torch.cat([x, extra_cond], dim=1), log_snr, cond_img, depth)#model为net
    ...
    @torch.no_grad()
    def sample(self, cond_img, depth, extra_cond=None, verbose=True):
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond_img, depth, extra_cond, verbose=True):
        self.history = []
        img = torch.randn(shape, device=self.device)
        conditioning_features = self.model.module.extract_features(cond_img)
        ...
        for i in tqdm(range(self.num_sample_steps), desc='sampling loop time step', total=self.num_sample_steps,
            ...
            img = self.p_sample(img, conditioning_features, depth, extra_cond, times, times_next)
    ...
    @torch.no_grad()
    def p_sample(self, x, cond, depth, extra_cond, time, time_next):
        batch, *_, device = *x.shape, x.device

        model_mean, model_variance = self.p_mean_variance(x=x, cond=cond, depth=depth, extra_cond=extra_cond, time=time,
                                                          time_next=time_next)
    ...
    def p_sample_g(self, x, cond, depth, extra_cond, time, time_next):
        batch, *_, device = *x.shape, x.device
    ...
    def p_mean_variance(self, x, cond, depth, extra_cond, time, time_next):
        ...
        pred = self.model.module.sample_unet(torch.cat([x, extra_cond], dim=1),
                                      batch_log_snr, cond, depth)
