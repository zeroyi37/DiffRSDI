...
def eval(mask_path='./RGBD_rail_datasets/test',
         pred_path='./results',
         dataset_name='RGBD_rail'):
    global mask_root, pred_root
    # for dataset in ['COD10K', 'CAMO', 'CHAMELEON']:
    for dataset in [dataset_name]:
        mask_root = os.path.join(mask_path, dataset, 'gt-test')
        pred_root = os.path.join(pred_path, dataset)
        mask_name_list = sorted(os.listdir(mask_root))

        res = process_map(measure_mea, mask_name_list, max_workers=1, chunksize=4)
