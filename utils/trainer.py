...
class Trainer(object):
    ...
    def save(self, epoch, max_to_keep=50):
        """
        ...
        last_save_name = str(self.results_folder / f'model-last-{epoch}.pt')
    ...
    def val(self, model, test_data_loader, accelerator, thresholding=False, save_to=None):
        ...
        for data in tqdm(test_data_loader, disable=not accelerator.is_main_process):
            image, depth, gt, name, image_for_post = data['image'], data['depth'], data['gt'], data['name'], data['image_for_post']
            gt = [np.array(x, np.float32) for x in gt]
            gt = [x / x.max() + 1e-8 for x in gt]
            image = image.to(device).squeeze(1)
            depth = depth.to(device).squeeze(1)
            ###################################################################


            out = self.train_val_forward_fn(model, image=image, depth=depth, verbose=False)
    ...
    def val_time_ensemble(self, model, test_data_loader, accelerator, thresholding=False, save_to=None):
        ...
        for data in tqdm(test_data_loader, disable=not accelerator.is_main_process):
            ...
            image = image.to(device).squeeze(1)
            depth = depth.to(device).squeeze(1)
            ensem_out = self.train_val_forward_fn(model, image=image, depth=depth, time_ensemble=True,
    ...
    def train(self):
        ...
        for epoch in range(self.cur_epoch, self.train_num_epoch):
            ...
            if (epoch + 1) % 1 == 0 or (epoch >= self.train_num_epoch * 0.7):
                mae, best_mae = self.val(self.model, self.test_loader, accelerator)#val_time_ensemble
